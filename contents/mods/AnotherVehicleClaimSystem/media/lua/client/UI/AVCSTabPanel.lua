...
AVCSTabPanel = ISTabPanel:derive("AVCSTabPanel")


function AVCSTabPanel:addView(name, icon, view)
	local viewObject = {}
	viewObject.name = name
    viewObject.icon = icon
	viewObject.view = view
	viewObject.tabWidth = 30 + self.tabPadX
...
function AVCSTabPanel:render()
	...
	for i,viewObject in ipairs(newViewList) do
        ...
        local icon = viewObject.icon
        local iconX = x + (tabWidth/4)
        local iconY = 0
        local iconWidth = 30
        local iconHeight = 30


        self:drawTextureScaled(icon, iconX, iconY, iconWidth, iconHeight, 1,1,1,1)
...
function AVCSTabPanel:new(x, y, width, height)

    local o = ISTabPanel:new(x, y, width, height)

    o.addView = AVCSTabPanel.addView
    o.render = AVCSTabPanel.render
